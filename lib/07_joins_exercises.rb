...
def ford_films
  ...
  JOIN
    castings ON movies.id = castings.movie_id
  WHERE
    actor_id = (SELECT id FROM actors WHERE name = 'Harrison Ford'
...
def ford_supporting_films
  ...
  JOIN
    castings ON movies.id = castings.movie_id
  WHERE
    ord != 1 AND actor_id = (SELECT id FROM actors WHERE name = 'Harrison Ford')
...
def andrews_films_and_leads
  ...
  JOIN
    actors ON actors.id = castings.actor_id
  WHERE ord = 1 AND title IN (SELECT
  title
  FROM
  movies
  JOIN
